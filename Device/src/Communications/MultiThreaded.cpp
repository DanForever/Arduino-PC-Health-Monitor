...
#pragma message "Multithreaded communications enabled"
...
Communications::MultiThreaded::MultiThreaded()
{
	Serial.begin(9600);
}

void Communications::MultiThreaded::Update()
{
	ReadIntoMainBuffer();
	ProcessMainBuffer();
}

void Communications::MultiThreaded::Ack(uint16_t id)
{
...
bool Communications::MultiThreaded::CouldBePattern(const char* pattern) const
{
...
void Communications::MultiThreaded::ClearMessage()
{
	GetMessage().Reset();
...
void Communications::MultiThreaded::ReadIntoMainBuffer()
{
...
void Communications::MultiThreaded::ProcessMainBuffer()
{
	while (m_queue.Count() >= MESSAGE_ARRAY_SIZE);
	...
	while (m_mainBuffer.Size() > 0)
	{
		...
		if (m_lookingForFooter)
		{
			...
			{
				GetWritableMessage() << m_parsingBuffer;
			}
...
void Communications::MultiThreaded::SendIdentity()
{
	uint8_t buffer[4];
...
void Communications::MultiThreaded::SendVersion()
{
	uint8_t buffer[4];
...
#else
#	pragma message ""Single threaded communications enabled""
